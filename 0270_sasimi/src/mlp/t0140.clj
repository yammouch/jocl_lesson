...
(defn -main [iter]
  (let [start-time (Date.)
        ...
        [tr ts] (map (comp (partial apply concat)
                     ...
                     p)
        ;_ (as-> (iterate first tr) x
        ;        (take-while coll? x)
        ;        (map count x)
        ;        (println x))
        ;_ (as-> (iterate first ts) x
        ;        (take-while coll? x)
        ;        (map count x)
        ;        (println x))
        ;_ (clojure.pprint/pprint (first tr))
        ;_ (clojure.pprint/pprint (spp/format-field (:field (first tr))))
