...
(defn fw [in]
  (let [{q :queue} @cl-env
    ...
    (cl/callk q dense-fw   nil [3] :m z :m in :m b :m w :i 3 :i 1)
...
(defn bw
 ...
 ([in label is-1st?]
  (let [{q :queue} @cl-env
    ...
    (cl/callk q cross-entropy-bw nil [3] :m v :m a :m label :f 0.1)
    (if is-1st?
      (do (cl/callk q dense-bw-m-ov nil [1 3] :m wacc :m in :m v :i 3)
          (CL/clEnqueueCopyBuffer q v bacc 0 0 (* 3 Sizeof/cl_float) 0 nil nil))
      (do (cl/callk q dense-bw-m    nil [1 3] :m wacc :m in :m v :i 3)
          (cl/callk q add           nil [3] :m bacc :m v)
          )))))
...
(defn run-subbatch [inputs labels]
  ...
  (let [{q :queue} @cl-env
    ...
    (cl/callk q sub nil [3] :m w :m wacc)
    (cl/callk q sub nil [3] :m b :m bacc)))
